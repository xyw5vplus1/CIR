...
def main():
    ...
    file = r'output_last_200/{}/non_equal_var.seed:{}/lambda_1:{}.lambda_3:{}/training.log'.format(args.dataset, args.seed, args.lambda_1, args.lambda_3)
    result = []
    result.append(args.lambda_1)
    result.append(args.lambda_3)
    result += read_log(file)[-4:]

    flag = os.path.isfile(r'output_last_200/result_{}.csv'.format(args.dataset))
    if flag==False:
        f = open('output_last_200/result_{}.csv'.format(args.dataset),'w',encoding='utf-8')
        csv_writer = csv.writer(f)
        csv_writer.writerow(['lambda_1','lambda_3','fdr','tpr','fpr','shd'])
    
    f = open('output_last_200/result_{}.csv'.format(args.dataset),'a+',encoding='utf-8')
    csv_writer = csv.writer(f)
