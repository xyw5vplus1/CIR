...
def main():
    ...
    file = r'output_hailfinder_1000/{}/non_equal_var.seed:1/lambda_1:{}.lambda_3:{}/training.log'.format(args.dataset, args.lambda_1, args.lambda_3)
    result = []
    result.append(args.lambda_1)
    result.append(args.lambda_3)
    result += read_log(file)

    flag = os.path.isfile(r'output_hailfinder_1000/result_{}(1-order).csv'.format(args.dataset))
    if flag==False:
        f = open('output_hailfinder_1000/result_{}(1-order).csv'.format(args.dataset),'w',encoding='utf-8')
        csv_writer = csv.writer(f)
        csv_writer.writerow(['lambda_1','lambda_3','fdr','tpr','fpr','shd'])
    
    f = open('output_hailfinder_1000/result_{}(1-order).csv'.format(args.dataset),'a+',encoding='utf-8')
    csv_writer = csv.writer(f)
