...
parser.add_argument('--lambda_CI',  type = float, default= 0.0,
                    help='coefficient for CI constraint.')
parser.add_argument('--CI_order',  type = int, default= 0,
                    help='highest for calculate CI matrix.')
parser.add_argument('--set_errors',  type = int, default= 0,
                    help='errors set by human.')
parser.add_argument('--hard_constraint', type = int, default = 0,
                    help='use CIC(1, hard constraint) or CIR(0, soft constraint)')
parser.add_argument('--significance_level_0',  type = float, default= 0.05,
                    help='significance level for CI in order 0.')
parser.add_argument('--significance_level_1',  type = float, default= 0.5,
...
if args.save_folder:
    ...
    folder_type = 'output_robust_'
    if args.hard_constraint == False:
        folder_type += 'CIR'
    else:
        folder_type += 'CIC'
    save_folder = folder_type + '({})/{}/seed:{}.errors:{}/samples:{} lambda_CI:{}/'.format(args.CI_order, args.dataset, args.seed, args.set_errors, args.data_sample_size, args.lambda_CI)
...
# file = r'skeleton_{}.csv'
# with open(file, encoding='utf-8') as f:
#     ground_truth_matrix = np.loadtxt(file, delimiter=',')
# ground_truth_G = nx.DiGraph(ground_truth_matrix)
# ================================================
# prepare conditional independence table(CI table) 
# ================================================
all_data -= np.mean(all_data, axis=0)
CI_table = prepare_CI_table(all_data, args.CI_order, args.significance_level_0, args.significance_level_1)

directed_graph = nx.to_numpy_array(ground_truth_G)!=0
un_skeleton = np.ones([args.data_variable_size, args.data_variable_size]) - np.eye(args.data_variable_size) - directed_graph
un_skeleton = un_skeleton * un_skeleton.transpose()
un_skeleton = np.ones([args.data_variable_size, args.data_variable_size]) - np.eye(args.data_variable_size) - un_skeleton

row, column = np.where(un_skeleton!=0) 
sz = len(row)
cnt = 0
while cnt<args.set_errors:
    ...
    while not (CI_table[x0][y0]==0):
        ...
        x0 = row[idx] 
        y0 = column[idx]
    CI_table[x0][y0] = CI_table[y0][x0] = 1
    cnt += 1

# Check the correctness of CI_table
print('Type II error edges: ',np.sum(un_skeleton * CI_table))
print('Type II error edges: ',np.sum(un_skeleton * CI_table),file=log)
#===================================
# load modules
#===================================
...
def train(epoch, best_val_loss, ground_truth_G, lambda_A, c_A, optimizer):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        # print('fdr:', fdr, 'tpr:', tpr, 'fpr:', fpr, 'shd:', shd, 'nnz:', nnz)
