...
def main():
    ...
    file = r'output_robust_CIC(0)/{}/seed:{}.errors:{}/samples:{} lambda_CI:{}/log.txt'.format(args.dataset, args.seed, args.error, args.samples, args.lambda_CI)
    result = []
    result.append(args.samples)
    result.append(args.lambda_CI)
    result += read_log(file)

    flag = os.path.isfile(r'output_robust_CIC(0)/result_{}_error_{}.csv'.format(args.dataset, args.error))
    if flag==False:
        f = open('output_robust_CIC(0)/result_{}_error_{}.csv'.format(args.dataset, args.error),'w',encoding='utf-8')
        csv_writer = csv.writer(f)
        # csv_writer.writerow(['samples','lambda_CI','fdr','tpr','fpr','shd'])
    
    f = open('output_robust_CIC(0)/result_{}_error_{}.csv'.format(args.dataset, args.error),'a+',encoding='utf-8')
    csv_writer = csv.writer(f)
