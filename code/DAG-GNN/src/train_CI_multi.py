...
if args.save_folder:
    ...
    if args.hard_constraint == False:
        ...
        folder_type += 'CIC'
    save_folder = folder_type + '({})\{}\seed{} tau{}\samples{} lambda_CI{}\\'.format(args.CI_order, args.dataset, args.seed, args.tau_A, args.data_sample_size, args.lambda_CI)
...
if args.dataset == 'Sachs':
    ...
    feat_train = torch.Tensor(all_data)
    feat_valid = torch.Tensor(all_data)
    feat_test = torch.Tensor(all_data)
...
def train(epoch, best_val_loss, ground_truth_G, lambda_A, c_A, optimizer):
    ...
    for batch_idx, (data, relations) in enumerate(train_loader):
        ...
        # QPM loss: 
        loss += 0.5 * c_A * h_A * h_A + 100. * torch.trace(origin_A*origin_A) + sparse_loss #+  0.01 * torch.sum(variance * variance)
